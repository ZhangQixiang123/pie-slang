...
`;
  ...
  while (Date.now() - startTime < TIMEOUT_MS) {
    ...
    try {
      // Try models in order; fall back on 503
      let result;
      for (const model of ["gemini-2.5-flash", "gemini-2.5-flash-lite"] as const) {
        try {
          result = await genAI.models.generateContent({ model, contents: prompt });
          break;
        } catch (err: unknown) {
          const msg = err instanceof Error ? err.message : String(err);
          if (msg.includes("503") || msg.includes("overloaded") || msg.includes("UNAVAILABLE")) {
            console.warn(`[todo-solver] ${model} unavailable, trying next...`);
            continue;
          }
          throw err;
        }
