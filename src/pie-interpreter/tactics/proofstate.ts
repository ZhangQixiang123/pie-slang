...
export class ProofState {
  ...
  addGoal(goals: GoalNode[]): void {
    ...
    // If multiple goals are created, set pendingBranches to require 'then' blocks
    if (goals.length > 1) {
      this.pendingBranches = goals.length;
    }
  }
  ...
  private PreviousGoalAux(curParent: GoalNode): GoalNode | null {
